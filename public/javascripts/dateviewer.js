...
DateViewer = Class.create({
	initialize : function(p_sBarId, p_sStartDate, p_sCenterDate, p_sEndDate, p_sStartDate2, p_sCenterDate2, p_sEndDate2, p_iStartYear, p_iEndYear, options) {
		...
		this.iLeftOffsetCenter = this.barStartDate.getDiffDays(this.centerDate)*this.options.dayWidth;			
		this.iLeftOffsetLH2 = this.barStartDate.getDiffDays(l_oStartDate2)*this.options.dayWidth;
		this.iLeftOffsetRH2 = this.barStartDate.getDiffDays(l_oEndDate2)*this.options.dayWidth;			
		this.iLeftOffsetCenter2 = this.barStartDate.getDiffDays(this.centerDate2)*this.options.dayWidth;			
	...
	dragShiftPanel : function() { 
		...
		// diff = $('centerhandle'). - $('lefthandle').offsetLeft;
	...
	_setDates : function() {
		...
		if (this.numberOfDays == null) {
		  ...
		  l_oDate3 = this.barStartDate.clone().addDays(l_iCenterPos);
		
		} else {
		  l_oDate2 = l_oDate.clone().addDays(this.numberOfDays);
		  l_oDate3 = l_oDate.clone().addDays(this.numberOfDays);
	...
	morphTo : function (p_oDateStart, p_oDateEnd) {
		...
		$('centerhandle').morph('left:'+l_offsetLeftCenter+'px');
	...
	cutOffStartToYear : function (p_oStartField, p_oEndField) {
		this.oStartField = p_oStartField;
		this.oEndField = p_oEndField;
		
		p_oStartField.setValue(Date.parse(p_oStartField.getValue()).toString('yyyy'));
		p_oEndField.setValue(Date.parse(p_oEndField.getValue()).toString(this.options.dateFormat));

		l_oStartDate = Date.parse(p_oStartField.getValue()+'-01-01');
		l_oEndDate = Date.parse(p_oEndField.getValue());	
		sliderReference.morphTo(l_oStartDate, l_oEndDate);
		
	},
	cutOffStartToMonth : function (p_oStartField, p_oEndField) {
		this.oStartField = p_oStartField;
		this.oEndField = p_oEndField;
		
		p_oStartField.setValue(Date.parse(p_oStartField.getValue()).toString('yyyy-MM'));
		p_oEndField.setValue(Date.parse(p_oEndField.getValue()).toString(this.options.dateFormat));

		l_oStartDate = Date.parse(p_oStartField.getValue()+'-01');
		l_oEndDate = Date.parse(p_oEndField.getValue());	
		sliderReference.morphTo(l_oStartDate, l_oEndDate);
	...
	_zoom : function(p_iFactor) {
		...
		l_oDateCenter	 = this.barStartDate.clone().addDays(l_iCenterPos);
		...
		this.iLeftOffsetCenter = this.barStartDate.getDiffDays(l_oDateCenter)*this.options.dayWidth;	
