...
Date.prototype.getDiffYears = function(p_oDate) {
	p_iOneDay = 1000*60*60*24*365;
...
/* The dateslider */
CenterDecadeDateViewer = Class.create({
	initialize : function(p_sBarId, p_sStartDate, p_sCenterDate, p_sEndDate, p_sStartDate2, p_sCenterDate2, p_sEndDate2, p_iStartYear, p_iEndYear, options) {
		...
		this.oEndField = null;
		this.oStartField2 = null;
		this.oEndField2 = null;
		...
		if (this.options.dragHandles == false) this.numberOfDays = this.oStartDate.getDiffYears(this.oEndDate);
		this.centerDate = Date.today();
		if (this.options.centerDate != null) this.centerDate = Date.parse(options['centerDate']);
		
		this.iLeftOffsetLH = this.barStartDate.getDiffYears(l_oStartDate)*this.options.dayWidth;
		this.iLeftOffsetRH = this.barStartDate.getDiffYears(l_oEndDate)*this.options.dayWidth;			
		this.iLeftOffsetCenter = this.barStartDate.getDiffYears(this.centerDate)*this.options.dayWidth;			
		this.iLeftOffsetLH2 = this.barStartDate.getDiffYears(l_oStartDate2)*this.options.dayWidth;
		this.iLeftOffsetRH2 = this.barStartDate.getDiffYears(l_oEndDate2)*this.options.dayWidth;			
		this.iLeftOffsetCenter2 = this.barStartDate.getDiffYears(this.centerDate2)*this.options.dayWidth;			
	...
	createSliderBar : function(p_sBarId) {
		...
		while(l_iYear <= this.iEndYear) {		
			...
			iTotalDays = 0;
			// (10).times(function(e) {
			// 	monthDivWidth =10; // l_oData.getDaysInMonth()*sliderReference.options.dayWidth;
		...
		l_shiftLeft = 0-this.barStartDate.getDiffYears(this.centerDate)*sliderDayDivWidth  +l_iCorrection;
		l_oFinishDate = Date.parse((this.iEndYear+1)+'-01-01');
		iBarWidth =this.barStartDate.getDiffYears(l_oFinishDate);
	    $(p_sBarId).setStyle({left : l_shiftLeft+'px', width : iBarWidth*sliderDayDivWidth+'px'}); 
		 $('spanbar').setStyle({left : l_shiftLeft+'px', width : iBarWidth*sliderDayDivWidth+'px'}); 
	...
	createHandles : function(p_sBarId, p_sStartDate, p_sEndDate) {
		...
		l_oCenterHandle = new Element('span', {className: 'centerHandle', id : 'centerhandle', style:'left:'+this.iLeftOffsetCenter+'px'}).update('&nbsp;');
		$(p_sBarId).appendChild(l_oCenterHandle);

		l_oLeftHandle2 = new Element('span', {className: 'leftHandle', id : 'lefthandle', style:'left:'+this.iLeftOffsetLH2+'px'}).update('&nbsp;');		
		l_oRightHandle2 = new Element('span', {className: 'rightHandle', id : 'righthandle', style:'left:'+this.iLeftOffsetRH2+'px'}).update('&nbsp;');
		
		$(p_sBarId).appendChild(l_oLeftHandle2);
		$(p_sBarId).appendChild(l_oRightHandle2);
		
		l_oCenterHandle2 = new Element('span', {className: 'centerHandle', id : 'centerhandle', style:'left:'+this.iLeftOffsetCenter2+'px'}).update('&nbsp;');
		$(p_sBarId).appendChild(l_oCenterHandle2);

		// create bottom span bar
		l_oLeftHandle = new Element('span', {className: 'leftHandle', id : 'lefthandle2', style:'left:'+this.iLeftOffsetLH+'px'}).update('&nbsp;');		
		l_oRightHandle2 = new Element('span', {className: 'rightHandle', id : 'righthandle2', style:'left:'+this.iLeftOffsetRH2+'px'}).update('&nbsp;');
		
		$('spanbar').appendChild(l_oLeftHandle);
		$('spanbar').appendChild(l_oRightHandle2);
		...
		// 								});		  	
		// 		//rwb
		// 		/* Make the center handler draggable */								 
		// 	  new Draggable(l_oCenterHandle,  {
		//   								snap: this.handleLimitPos,
		// 								containment: p_sBarId,
		// 								constraint:'horizontal',
		// 								onDrag : sliderReference._centerDrag,
		// 								onEnd : sliderReference._centerDrag
		// 								});		  	
		// 	  
		//   } else {
		...
		// 	//rwb
		// 	l_oCenterHandle.setStyle({opacity:.01,cursor:'pointer'});
		// 	
		//   }		 
	},
	...
	createShiftPanel : function(p_sBarId, p_sStartDate, p_sEndDate) {
		...
		l_iBarWidth2 = (this.iLeftOffsetRH2-this.iLeftOffsetLH2)+(2*this.sliderBarMargin);

		l_oShiftPanel2 = new Element('div', {id : 'shiftpanel2', style:'left:'+(this.iLeftOffsetLH2)+'px; width:'+l_iBarWidth2+'px'});
		$(p_sBarId).appendChild(l_oShiftPanel2);		

		// span bar
		l_iBarWidth = (this.iLeftOffsetRH2-this.iLeftOffsetLH)+(2*this.sliderBarMargin);
		l_oShiftPanel = new Element('div', {id : 'spanshiftpanel', style:'left:'+(this.iLeftOffsetLH)+'px; width:'+l_iBarWidth+'px'});
		$('spanbar').appendChild(l_oShiftPanel);		
		...
		// 							  onDrag:function() { 
		// 									/* Set the handlers while dragging the shiftpanel */
		// 									var diff = $('centerhandle').offsetLeft - $('lefthandle').offsetLeft;
		// 									$('centerhandle').setStyle({left: ($('shiftpanel').offsetLeft + diff -sliderReference.sliderBarMargin)+'px'});							
		// 
		// 									$('lefthandle').setStyle({left: ($('shiftpanel').offsetLeft-sliderReference.sliderBarMargin)+'px'});
		// 									$('righthandle').setStyle({left: ($('shiftpanel').offsetLeft + $('shiftpanel').offsetWidth-sliderReference.sliderBarMargin)+'px'});						
		// 									sliderReference._setDates();
	...
	_setDates : function() {
		...
		l_iCenterPos = $('centerhandle').offsetLeft/this.options.dayWidth;
		
		l_oDate = this.barStartDate.clone().addYears(l_iLeftPos);
		if (this.numberOfDays == null) {
		  l_oDate2 = this.barStartDate.clone().addYears(l_iRightPos);
		  l_oDate3 = this.barStartDate.clone().addYears(l_iCenterPos);
		} else {
		  l_oDate2 = l_oDate.clone().addYears(this.numberOfDays);
		  l_oDate3 = l_oDate.clone().addYears(this.numberOfDays);
		} 
	
		if(this.oStartField && this.oEndField && this.oCenterField) {
			...
			// this.oCenterField.setValue(l_oDate3.toString(this.options.dateFormat));		
		}	
	},
	...
	morphTo : function (p_oDateStart, p_oDateEnd) {
		l_offsetLeftLH = this.barStartDate.getDiffYears(p_oDateStart)*this.options.dayWidth;
		l_offsetLeftRH = this.barStartDate.getDiffYears(p_oDateEnd)*this.options.dayWidth;
	...
	//rwb
	attachFields : function (p_oStartField, p_oEndField, p_oCenterField) {
		this.oStartField = p_oStartField;
		this.oEndField = p_oEndField;
		this.oCenterField = p_oCenterField;
		...
		// p_oCenterField.setValue(this.centerDate.toString(this.options.dateFormat));
	...
	_zoom : function(p_iFactor) {
		...
		l_oDateStart = this.barStartDate.clone().addYears(l_iLeftPos);
		l_oDateEnd	 = this.barStartDate.clone().addYears(l_iRightPos);
		l_oDateCenter	 = this.barStartDate.clone().addYears(l_iCenterPos);
				
		this._removeSliderBar();
		this.options.dayWidth = this.options.dayWidth+p_iFactor;
		
		this.iLeftOffsetLH = this.barStartDate.getDiffYears(l_oDateStart)*this.options.dayWidth;
		this.iLeftOffsetRH = this.barStartDate.getDiffYears(l_oDateEnd)*this.options.dayWidth;	
		this.iLeftOffsetCenter = this.barStartDate.getDiffYears(l_oDateCenter)*this.options.dayWidth;	
	...
	nudge_left : function(element) {
		$(element).setStyle({left:$(element).offsetLeft-4+'px'});
		l_panelLength = $('righthandle').offsetLeft - $('lefthandle').offsetLeft - 4;
		$('shiftpanel').setStyle({left: ($('lefthandle').offsetLeft+4)+'px', width : l_panelLength+'px'});															
		sliderReference._setDates();
	},
